...
serve(async (req) => {
  ...
  try {
    ...
    if (!rawText?.trim()) return new Response(JSON.stringify({ error: "No text provided to refine" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    const prompt = `You are a helpful cooking assistant for the Arnesson family recipe book. Take this raw spoken recipe description and convert it into a well-structured recipe. The input may be in English or Swedish â€” always respond in the SAME language as the input.

"${rawText}"

Return a JSON object with:
- title: A clear, appetizing recipe title
...
Only return the JSON object, no markdown or other text.`;
    ...
    if (!content) return new Response(JSON.stringify({ error: "Failed to refine recipe" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Failed to parse refined recipe" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
